...
def eval(args, accelerator, model, tokenizer, eval_dataloader, postprocess_text, gen_kwargs, gen_type):
    ...
    try:
        ...
        with open(os.path.join(args.output_dir, f"eval_rouge_{gen_type}.json"), 'w') as f:
...
def main():
    ...
    for gen_type in gen_list.keys():
        ...
        if rouge is not None:
            ...
            if rouge['rouge-1']['f'] > 0.22 and rouge['rouge-2']['f'] > 0.085 and rouge['rouge-l']['f'] > 0.205:
                logger.info('passed baseline')
        logger.info(f'**************************')
