...
def parse_args():
    ...
    parser.add_argument(
        "--num_train_epochs_to_eval",
        type=int,
        default=1,
        help="Eval after number of train epochs.",
    )
    parser.add_argument(
        "--num_warmup_epochs_to_eval",
        type=int,
        default=0,
        help="Eval after number of train epochs.",
    )
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of max train samples.",
    )
    parser.add_argument(
        "--max_eval_samples",
        type=int,
...
def main():
    ...
    if 'ntuadlhw2' in args.dataset_name:
        raw_datasets = load_dataset(args.dataset_name, data_files={'train':'train.jsonl','validation':'public.jsonl'})
    ...
    if args.max_train_samples and isinstance(args.max_train_samples, int):
        raw_datasets['train'] = raw_datasets['train'].select(range(args.max_train_samples))

    if args.max_eval_samples and isinstance(args.max_eval_samples, int):
        raw_datasets['validation'] = raw_datasets['validation'].select(range(args.max_eval_samples))

    if args.debug:
        ic(raw_datasets.keys())
        ic(raw_datasets['train'])
        ic(raw_datasets['validation'])
    ...
    if args.use_sgd:
        ...
        optimizer = optim.Adafactor(optimizer_grouped_parameters, lr=args.learning_rate)
    else:
    ...
    progress_bar = tqdm(range(args.max_train_steps), disable=not accelerator.is_local_main_process, position=0, leave=False)
    completed_steps = 0
    starting_epoch = 0
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if epoch == (args.num_train_epochs - 1) or (epoch != 0 and (epoch + 1) > args.num_warmup_epochs_to_eval and ((epoch + 1) % args.num_train_epochs_to_eval == 0)):
            ...
            df.to_json(os.path.join(args.output_dir, f"eval_results_epoch{epoch}.jsonl"), orient='records', lines=True)

            from tw_rouge import get_rouge
            rouge = get_rouge(all_decoded_preds, all_decoded_labels)
        ...
        if args.with_tracking:
            ...
            if rouge is not None:
                result['tw/rouge-1'] = rouge['rouge-1']
                result['tw/rouge-2'] = rouge['rouge-2']
                result['tw/rouge-l'] = rouge['rouge-l']
